...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    super.initState();
    _load();
    _checker = Timer.periodic(const Duration(seconds: 10), (_) => _checkTimers());
  }
  ...
  Future<void> _deleteAccount(ClaudeAccount acc) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('Delete "${acc.nickname}"?', style: const TextStyle(color: Colors.grey)),
        actions: [
  ...
  int get _readyCount => _accounts.where((a) => a.isReady && a.timerStartedAt != null).length;
  ...
  Widget build(BuildContext ctx) => Scaffold(
        ...
        child: Row(children: [
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            RichText(text: const TextSpan(children: [
              TextSpan(text: 'CLAUDE', style: TextStyle(color: Color(0xFF00FF88), fontSize: 22, fontWeight: FontWeight.bold, fontFamily: 'monospace', letterSpacing: 2)),
              TextSpan(text: 'LINK', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold, fontFamily: 'monospace', letterSpacing: 2)),
            ])),
            const Text('Session Manager', style: TextStyle(color: Colors.grey, fontSize: 11, letterSpacing: 1)),
          ]),
          const Spacer(),
          // Stats
      ...
      if (_accounts.isNotEmpty)
        Container(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
            ...
            _summaryItem('IDLE', '${_accounts.length - _readyCount - _waitingCount}', Colors.grey),
          ]),
        ),
    ...
    floatingActionButton: FloatingActionButton.extended(
      ...
      label: const Text('ADD ACCOUNT', style: TextStyle(fontWeight: FontWeight.bold, fontFamily: 'monospace')),
    ),
  );
  ...
  Widget _statBadge(String emoji, String count, Color color) => Container(
    ...
    child: Text('$emoji $count', style: TextStyle(color: color, fontSize: 12, fontWeight: FontWeight.bold)),
  );
  ...
  Widget _emptyState() => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    ...
    const Text('No accounts yet', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
    const SizedBox(height: 8),
    const Text('Tap + to add your first Claude account', style: TextStyle(color: Colors.grey, fontSize: 13)),
